    ...
    try:
        while True:
            ...
            if msg_type == "AT*SENDIM_RESPONSE":
                ...
                logging.info("  ✓ Position request delivered to transceiver")
            
            elif msg_type == "FAILEDIM":
                logging.error("  ✗ Position request FAILED - will retry")
                ...
                if data["valid"]:
                    ...
                    if msg and "e" in msg and "n" in msg and "u" in msg:
                        ...
                        logging.warning("  ⚠️  Received message without position data: {}".format(msg))
                else:
                    logging.warning("  ⚠️  Low integrity message (integrity={})".format(data['integrity']))
    ...
    try:
        while True:
            ...
            try:
                ...
                logging.warning("  ⏱️  TIMEOUT waiting for position response")
                state["waiting_for_response"] = False
...
async def main():
    ...
    logging.info("✓ Connected to USBL at {}:{}".format(USBL_IP, USBL_PORT))
