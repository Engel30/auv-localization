...
async def send_response_to_vehicle():
    ...
    # Aggiungi marker '$' per identificare inizio payload
...
async def response_listener(reader):
    ...
    try:
        while True:
            ...
            if msg_type not in ["OK", "EMPTY", "OTHER", "SENDSTART", "SENDEND", "RECVSTART", "RECVEND"]:
                print("[DEBUG] Received: {} -> {}".format(msg_type, data))
