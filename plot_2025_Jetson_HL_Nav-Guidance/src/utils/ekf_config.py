...
# ============================================================================
# These variables control the graphical elements drawn on the 2D trajectory map.
# All coordinates are in meters, in the same reference frame as the EKF state.

# NOTE: BOA_COORDINATES, NORTH_MARKER_COORDINATES and POOL_RADIUS are defined
# in the "PARAMETRI VISUALIZZAZIONE E RIFERIMENTI" section above and are
# imported from there by ekf_sensor_fusion.py.

# --- Rectangle obstacle / structure / zone of interest ---
# The rectangle is centred at RECT_CENTER, then rotated by RECT_ANGLE.
# Set SHOW_RECTANGLE = False to hide it without deleting the parameters.
SHOW_RECTANGLE = True            # Draw the rectangle on the 2D map
RECT_CENTER    = [-8.0, 0.0]     # Centre of the rectangle [x, y] in metres
RECT_WIDTH     = 2.5             # Width  of the rectangle along its local X axis [m]
                                 # (measured before any rotation)
RECT_HEIGHT    = 1.5             # Height of the rectangle along its local Y axis [m]
                                 # (measured before any rotation)
RECT_ANGLE     = 0.0             # Rotation angle in degrees, counterclockwise from East
                                 # (0 = aligned with world axes, 90 = long side points North)
RECT_COLOR     = 'orange'        # Matplotlib colour name or hex string for fill and edge
RECT_LABEL     = 'Obstacle'      # Label shown for this shape in the map legend

# --- Reference points ---
# List of notable positions to mark on the 2D map with a diamond marker + text label.
# Each entry: [x_metres, y_metres, 'Label string']
# Add or remove rows freely — the code will iterate over however many entries you provide.
REFERENCE_POINTS = [
    [-3.0,  7.5, "Ref A"],   # Reference to the North  (0 m E,  5 m N)
    [-3.0,  -7.5, "Ref B"],   # Reference to the East   (5 m E,  0 m N)
    [ 3.0,  7.5, "Ref C"],   # Reference to the West   (5 m W,  0 m N)
]
