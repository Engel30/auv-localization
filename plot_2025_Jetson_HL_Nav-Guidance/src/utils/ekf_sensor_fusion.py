...
if __name__ == "__main__":
    ...
    # --- Visualization config (safe defaults if ekf_config.py is absent) ---
    ...
    try:
        ...
        pass  # defaults defined above are used
    ...
    # Layout: the 2D map occupies the entire left column (dominant).
    # USBL range and depth profile are stacked in a narrower right column
    # as secondary context panels.
    fig = plt.figure(figsize=(16, 8))
    gs = fig.add_gridspec(1, 2, width_ratios=[2.8, 1], wspace=0.28)

    # ---- PLOT 1: 2D Trajectory Map (dominant) ----
    ax1 = fig.add_subplot(gs[0], aspect='equal')

    # Pool / arena circular boundary
    pool = Circle((0, 0), POOL_RADIUS, fill=False, edgecolor='blue',
                  linewidth=2, linestyle='--', label=f'Pool (R={POOL_RADIUS:.0f} m)')
    ax1.add_patch(pool)

    # BOA (USBL transponder) — use the same position the EKF is using, not a
    # separate hardcoded coordinate, so the marker is always consistent with
    # the filter.  A triangle marker is large and easy to spot on the map.
    boa_xy = (boa_position[0], boa_position[1]) if boa_position is not None \
             else BOA_COORDINATES
    ax1.plot(boa_xy[0], boa_xy[1], 'r^',
             markersize=16, zorder=7,
             label='BOA (transponder)',
             markeredgecolor='darkred', markeredgewidth=1.5)
    ax1.annotate('BOA', xy=boa_xy,
                 xytext=(boa_xy[0] + 0.4, boa_xy[1] + 0.4),
    ...
    # Configured in ekf_config.py via SHOW_RECTANGLE, RECT_CENTER, RECT_WIDTH,
    # RECT_HEIGHT, RECT_ANGLE, RECT_COLOR, RECT_LABEL.
    # The patch is drawn centred at RECT_CENTER and then rotated by RECT_ANGLE
    # degrees counterclockwise using an Affine2D transform so the rotation
    # happens around the rectangle's own centre (not the plot origin).
    if SHOW_RECTANGLE:
        rect_patch = Rectangle(
            # Lower-left corner = centre minus half dimensions (before rotation)
            (RECT_CENTER[0] - RECT_WIDTH / 2,
             RECT_CENTER[1] - RECT_HEIGHT / 2),
            RECT_WIDTH,      # width  along local X axis (pre-rotation) [m]
            RECT_HEIGHT,     # height along local Y axis (pre-rotation) [m]
            linewidth=2,
        ...
        # Build the rotation transform and compose it with the axes data transform
        t = mtransforms.Affine2D().rotate_deg_around(
            RECT_CENTER[0], RECT_CENTER[1], RECT_ANGLE
        ) + ax1.transData
        rect_patch.set_transform(t)
        ax1.add_patch(rect_patch)

    # ---- Reference points ----
    # Each entry in REFERENCE_POINTS is [x, y, 'Label'].
    # All points share the same diamond marker style; labels are placed with a
    # small offset so they do not overlap the marker itself.
    for rp in REFERENCE_POINTS:
        rx, ry, rlabel = float(rp[0]), float(rp[1]), str(rp[2])
        ax1.plot(rx, ry, 'D', color='purple', markersize=9, zorder=6)
        ax1.annotate(rlabel, xy=(rx, ry),
                     xytext=(rx + 0.25, ry + 0.25),
                     fontsize=8, color='purple', fontweight='bold')
    # Single proxy artist so all reference points share one legend entry
    if REFERENCE_POINTS:
        ax1.plot([], [], 'D', color='purple', markersize=9,
                 label='Reference points')

    # EKF trajectory colored by elapsed time
    x_new    = trajectory[:, 1]
    y_new    = trajectory[:, 2]
    ...
    # ---- PLOT 2 & 3: secondary context panels stacked in the right column ----
    ...
    # EKF-estimated range from robot to BOA
    if boa_position is not None:
        dx_ekf = x_new - boa_position[0]
        dy_ekf = y_new - boa_position[1]
        dz_ekf = trajectory[:, 3] - boa_position[2]
        range_ekf = np.sqrt(dx_ekf**2 + dy_ekf**2 + dz_ekf**2)
    else:
    ...
    ax2.axhline(y=np.mean(usbl_range), color='blue', linestyle='--',
                alpha=0.3, linewidth=1, label='Mean range')
    ...
    # Depth (Z component of the EKF state) on the vertical Y-axis.
    # Y-axis is inverted so that increasing depth points downward, matching
    # the marine convention (surface = top of the plot).
    ax3 = fig.add_subplot(gs_right[1])

    depth = trajectory[:, 3]   # Z component of EKF state [m]

    ax3.plot(time_new, depth, '-', color='teal', linewidth=1.5)
    ax3.fill_between(time_new, depth, alpha=0.15, color='teal')
    ax3.invert_yaxis()  # Depth increases downward (marine / oceanographic convention)
    ...
    # --- Final statistics (terminal output) ---
    print(f"\n=== STATISTICHE FINALI ===")
    print(f"\nTraiettoria EKF:")
    ...
    # USBL range error at each fix timestamp
    range_errors = []
    for i, t_usbl in enumerate(time_usbl):
        idx = np.argmin(np.abs(time_new - t_usbl))
        if boa_position is not None:
            dx_i = x_new[idx] - boa_position[0]
            dy_i = y_new[idx] - boa_position[1]
            dz_i = trajectory[idx, 3] - boa_position[2]
            range_ekf_at_usbl = np.sqrt(dx_i**2 + dy_i**2 + dz_i**2)
        else:
            range_ekf_at_usbl = np.sqrt(
                x_new[idx]**2 + y_new[idx]**2 + trajectory[idx, 3]**2)
        range_errors.append(abs(range_ekf_at_usbl - usbl_range[i]))
